...
def _detect_source_language(text: str) -> Optional[str]:
    ...
    # Vietnamese with diacritics is highly reliable.
    if _VI_DIACRITIC_RE.search(t):
        return "vi"

    # Latin no-diacritic heuristic: vi (khong dau) vs en
...
def _language_guardrail_explanation(language: str) -> str:
    ...
    # Tránh sinh explanation sai ngôn ngữ khi chưa có template bản địa hóa.
    return ""
...
def _force_minimal_rewrite(text: str, goal: str = "", language: str = "vi") -> str:
    ...
    # English grammar fallback for short/simple sentences
    if language == "en":
...
def _same_text_explanation(language: str) -> str:
    if language == "en":
        return "AI output was too similar to the original text, so the system applied a fallback rewrite for a clearer difference."
    if language == "vi":
        return "AI trả về nội dung quá giống bản gốc; hệ thống đã áp dụng rewrite dự phòng để tạo phiên bản khác biệt rõ ràng."
    return ""
...
async def rewrite_paragraph(
    ...
    # If document_id is provided, verify ownership and resolve source paragraph text from DB.
    if payload.document_id:
        ...
        if paragraph:
            ...
            # Legacy docs có thể thiếu bảng paragraphs; fallback sang payload text
            original_text = payload_original_text
        elif doc.raw_text and doc.raw_text.strip():
            # Fallback cuối: lấy từ raw_text của document
    ...
    # Ưu tiên giữ ngôn ngữ gốc của đoạn văn; chỉ fallback sang ngôn ngữ hồ sơ khi không nhận diện được.
    ...
    # Guardrail: nếu output AI không khác input thì áp fallback để luôn có cải thiện nhìn thấy.
    ...
    # Guardrail ngôn ngữ: đảm bảo output bám theo ngôn ngữ người dùng đã chọn.
